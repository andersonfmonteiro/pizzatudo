...
async function main() {
    ...
    try {
        ...
        const productsCollection = client.db("pizzatudo").collection("products");
        const categoriesCollection = client.db("pizzatudo").collection("categories");

        let categories = ['café', 'almoço', 'jantar', 'petiscos'].map((category) => { return { name: category } });
